...
class TTSWorker:
    ...
    def __init__(self, cache_dir: str = "./cache/tts"):
        ...
        self._sample_rate = self._tts_config.get(
            "sample_rate", 24000
        )  # Kokoro uses 24 kHz
        self._voices = self._tts_config.get(
            "voices", {"dj_a": "af_sarah", "dj_b": "am_michael"}
        )
    ...
    def _log_to_test_file(self, msg: str):
        try:
            ...
            with open("tts_log.txt", "a", encoding="utf-8") as f:
                f.write(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ⚙️ TTS: {msg}\n"
                )
        except:
            pass
    ...
    def cleanup_cache(self) -> None:
        ...
        try:
            ...
            if deleted > 0:
                logger.info(
                    f"TTS cache cleanup: deleted {deleted} files, freed {freed / (1024 * 1024):.1f} MB"
                )
    ...
    async def synthesize(self, line: ScriptLine) -> TTSResult:
        ...
        # Rule: Priority to the global language setting
        current_lang = config.get("language", "auto").lower()
        ...
        if current_lang == "ru":
            ...
            # Auto mode: check for Cyrillic letters
        ...
        if self._cache_enabled and cached_path.exists():
            logger.info(
                f"Cache hit TTS: speaker={line.speaker} voice={voice_id} is_ru={is_ru} text={line.text[:40]}..."
            )
            return TTSResult(
                line=line,
        ...
        logger.info(
            f"TTS request: speaker={line.speaker} voice={voice_id} is_ru={is_ru} text={line.text[:60]}..."
        )
        ...
        if is_ru:
            # 1a. Vosk HTTP API
            if result is None:
                result = await self._try_vosk_http(
                    line.text, int(voice_id), output_path
                )
        else:
            # 1b. Kokoro HTTP API
            if result is None:
                result = await self._try_kokoro_http(
                    line.text, str(voice_id), output_path
                )
    ...
    async def synthesize_batch(self, lines: List[ScriptLine]) -> List[TTSResult]:
        """Synthesize multiple lines in parallel."""
        tasks = [self.synthesize(line) for line in lines]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        
        final_results = []
        for r in results:
            if isinstance(r, Exception):
                logger.error(f"Batch TTS Exception: {r}")
                continue
            if r is not None:
        ...
        try:
            ...
            if isinstance(dj_list, list):
                for dj in dj_list:
                    if (
                        isinstance(dj, dict)
                        and (dj.get("id") or "").lower() == speaker_key
                    ):
    ...
    async def _try_kokoro_http(
        ...
        try:
            ...
            logger.info(f"[DEBUG] TTS POST start: {self._kokoro_url}/tts (voice={voice_id})")
            ...
            logger.info(f"[DEBUG] TTS POST end: status={resp.status_code}")
            ...
            logger.info(
                f"Kokoro TTS OK: {duration:.1f}s  voice={voice_id}  '{text[:30]}...'"
            )
            self._log_to_test_file(
                f"✅ Kokoro OK [{voice_id}] (Duration: {duration:.1f}s)"
            )
    ...
    # ──────────────────────────────────────────────────────────
    async def _try_vosk_http(
        self, text: str, voice_id: int, output_path: str
    ) -> Optional[Dict]:
        try:
            ...
            if resp.status_code != 200:
                ...
                # Ensure we don't return partial or error data as audio
                return None

            # CRITICAL SAFETY CHECK: Ensure the response is actual WAV audio, not error JSON
            if not resp.content.startswith(b"RIFF"):
                logger.error(f"Vosk returned NON-WAV content (Pique prevention): {resp.content[:100]}")
                return None
            ...
            duration = self._get_duration(output_path)
            logger.info(
                f"Vosk TTS OK: {duration:.1f}s  voice={voice_id}  '{text[:30]}...'"
            )
            self._log_to_test_file(
                f"✅ Vosk OK [ID:{voice_id}] (Duration: {duration:.1f}s)"
            )
            return {"path": output_path, "duration": duration}
    ...
    async def _try_pyttsx3(
        ...
        def _synth():
            ...
            for voice in voices:
                ...
                if (
                    target_voice in voice_name.lower()
                    or voice_id_attr.lower() == voice_id.lower()
                ):
            ...
            if selected_voice:
                try:
                    ...
                    logger.info(
                        f"pyttsx3 using voice: {getattr(selected_voice, 'name', 'unknown')}"
                    )
                except Exception as e:
                    logger.warning(f"pyttsx3 could not set voice: {e}")
            else:
                logger.warning(
                    f"pyttsx3 could not find voice {voice_id}, using default"
                )
    ...
    async def _try_ffmpeg_tone(self, text: str, output_path: str) -> Optional[Dict]:
        ...
        cmd = [
            ...
            "lavfi",
            "-i",
            f"sine=frequency={freq}:duration={duration}:sample_rate={self._sample_rate}",
            "-af",
            f"volume=0.08,afade=t=in:d=0.1,afade=t=out:st={max(duration - 0.2, 0.1):.2f}:d=0.2",
            output_path,
        ]
